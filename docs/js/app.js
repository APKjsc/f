...
(function (process){
  ...
  page.back = function(path, state) {
    if (page.len > 0) {
        console.log("page.js - back: history.back");
      ...
      setTimeout(function() {
          console.log("page.js - back: force to: "+ path);
        page.show(path, state);
      });
    }else{
      setTimeout(function() {
          console.log("page.js - back: fallback using base: "+ path);
...
var app = (function App() {
	...
	function init() {
		...
		if (bowser.msie && bowser.version <= 10 || bowser.android && bowser.version <= 4 || bowser.ios && bowser.version <= 8.3 || bowser.firefox && bowser.version <= 40) {
			showBackup();
			return false;
		}
		...
		mobileTimeline = new MobileTimeline();
		mobileTimeline.show();

		// setup router (page.js)
		page('/blog/:blog', showWork);
		page.exit('/blog/:blog', hideWork);
		page('/', showTimeline);
		page.exit('/', hideTimeline);
		page('/search', showSearch);
		page.exit('/search', hideSearch);
		page('/milestones', showMilestones);
		page.exit('/milestones', hideMilestones);
		page('/leaders', showLeaders);
		page.exit('/leaders', hideLeaders);
		page('/about', showAbout);
		page.exit('/about', hideAbout);
		page.start();
		...
		initLargeNav();
		initSmallNav();
		svg4everybody(); // IE external SVG fix

		window.addEventListener('resize', requestResize); // listen for resizes in one place
	}
 ...
 */
	function initLargeNav() {
		...
		for (var i = 0; i < headerLinks.length; i++) {
			headerLinks[i].addEventListener('click', gotoSection);
		}

		// footer share links
		var footerLinks = document.querySelectorAll('footer a');
		for (var j = 0; j < footerLinks.length; j++) {
			footerLinks[j].addEventListener('click', share);
		}

		// logo goes back to the timeline
 ...
 */
	function initSmallNav() {
		...
		hamburger.addEventListener('click', function (e) {
			...
			var mobilemenu = document.body.querySelector('.mobile-menu');
			var bg = document.body.querySelector('.mobile-menu-bg');
			var ul = document.body.querySelector('.mobile-menu ul');
			...
			if (document.body.classList.contains("open-menu")) {
				...
				if (height <= 400) {
					setTimeout(function () {
						...
						mobilemenu.style.background = "#c52127";
		...
		menuLinks = document.querySelectorAll('.mobile-menu a');
		Array.prototype.forEach.call(menuLinks, function (el, i) {
			el.addEventListener('click', function (e) {
				e.preventDefault();

				var loc = e.currentTarget.getAttribute('href');
				var era = e.currentTarget.getAttribute('data-era');
 ...
 ***************************************************************************/
	function share(e) {
		e.preventDefault();
		console.log("share");
		window.open(e.currentTarget.getAttribute('href'), 'share50', 'width=500,height=500');
	}
	...
	function go(path, data, force) {
		console.log('go: ' + path);
		if (path !== page.current || force === true) {
			page(path, data);
		}
	}
 ...
 *                                                                          *
 ***************************************************************************/
	function showTimeline(ctx, next) {
		// if the current route is the timeline, close and open Eras (desktop only)
    var forceClose = page.current == '/' && page.current == page.previous;

		timeline.show(forceClose);
		mobileTimeline.show(ctx);
	}

	function hideTimeline(ctx, next) {
		if (timeline) {
			timeline.hide(next);
		}
		if (mobileTimeline) {
			mobileTimeline.hide(next);
		}
	}

	function showWork(ctx, next) {
		if (!detail) {
			var options = {};

			if (timeline) {
				options.enter = timeline.enterDetailView;
				options.exit = timeline.exitDetailView;
			}

			detail = new DetailView(options);
			detail.show(ctx.params.blog, ctx);
		} else {
			detail.show(ctx.params.blog, ctx);
		}
	}

	function hideWork(ctx, next) {
		if (detail) {
			detail.hide(next);
		} else {
			if (next) {
	...
	function hideSearch(ctx, next) {
		if (search) {
			search.hide(ctx, next);
		}
	}

	function showMilestones(ctx, next) {
		if (!milestones) {
			milestones = new MilestoneView();
			milestones.show();
		} else {
			milestones.show();
		}
	...
	function hideMilestones(ctx, next) {
		if (milestones) {
			milestones.hide(ctx, next);
		}
	}

	function showLeaders(ctx, next) {
		if (!leaders) {
			leaders = new LeaderView();
			leaders.show();
		} else {
			leaders.show();
		}
	...
	function hideLeaders(ctx, next) {
		if (leaders) {
			leaders.hide(ctx, next);
		}
	}

	function showAbout(ctx, next) {
		if (!about) {
			about = new AboutView();
			about.show();
		} else {
 ...
 ***************************************************************************/
	...
	function resize() {
		...
		if (mobileTimeline) mobileTimeline.resize();
		if (search) search.resize();
		if (detail) detail.resize();
		if (milestones) milestones.resize();
		if (leaders) leaders.resize();
	}
 ...
 ***************************************************************************/
	function showBackup() {
		...
		var oldBrowserhtml = document.createElement('p');
		oldBrowserhtml.classList.add("old-browser");
		oldBrowserhtml.innerHTML = "Unfortunately your browser is out of date. Please update to view this website. For more information and links to upgrade, please visit <a href='http://browsehappy.com'>browsehappy.com</a>. <br><br>Keep up with our latest work and news at <a href='http://martinagency.com'>martinagency.com</a>.";
		document.body.innerHTML = "";
		document.body.appendChild(oldBrowserhtml);
	}
...
function AboutView(options) {
	...
	function init() {
		node = document.querySelector('.about-view');
		closeBtn = node.querySelector('.close');
	...
	function animate() {
		var p0 = new SplitText('.about-view .content .p0', { type: 'words' });
		var p1 = new SplitText('.about-view .content .p1', { type: 'words' });
		var p2 = new SplitText('.about-view .content .p2', { type: 'words' });
		var p4 = new SplitText('.about-view .content .p4', { type: 'words' });
		var p5 = new SplitText('.about-view .content .p5', { type: 'words' });

		tl = new TimelineLite();
		tl.set($('.about-view .content'), { opacity: 1 });

		var width = window.innerWidth || document.documentElement.clientWidth;
		if (width > 768) {
			TweenLite.from(closeBtn, 1, { x: 800, opacity: 0, ease: Power4.easeInOut });
		}

		tl.staggerFrom(p0.words, 1, { opacity: 0 }, 0.03, '+=1');
		tl.staggerFrom(p1.words, 1, { opacity: 0 }, 0.03, '-=0.7');
		tl.staggerFrom(p2.words, 1, { opacity: 0 }, 0.03, '-=0.7');
		tl.staggerFrom(p4.words, 1, { opacity: 0 }, 0.03, '-=0.7');
		tl.staggerFrom(p5.words, 1, { opacity: 0 }, 0.03, '-=0.7');
	}

	function show() {
		$(window).scrollTop(0);
		visible = true;
		document.body.setAttribute('data-state', 'about');
...
function IntroView(options) {
	...
	/*
 * Method that ensures the intro is only played once per user
 */
	function init() {
    ...
    if (page.current != '/') {
			if (!hasCookie("visited")) {
				...
				console.log('grrr2');
				document.body.classList.remove("show-intro");
				document.body.classList.remove("show-intro-2");
			}
		} else {
			pickPath();
		}

		setCookie("visited", "true", 0.02);
		setCookie('seenTextIntro', 'true', 50);
	}
	...
	function firstPart() {
		var p0 = new SplitText('.intro .part1 .p0', { type: 'words' });
		var p1 = new SplitText('.intro .part1 .p1', { type: 'words' });
		var p2 = new SplitText('.intro .part1 .p2', { type: 'words' });
		var div = document.querySelector('.intro .part1 .divider div');
		var p3 = new SplitText('.intro .part1 .p3', { type: 'words' });

		tl = new TimelineLite();
		tl.set($('.intro .part1'), { opacity: 1 });

		tl.staggerFrom(p0.words, 1, { opacity: 0 }, 0.05, '+=0.1');
		tl.staggerFrom(p1.words, 1, { opacity: 0 }, 0.05, '+=0.7');
		tl.staggerFrom(p2.words, 1, { opacity: 0 }, 0.05, '+=0');
		tl.to(div, 0.8, { width: 50, ease: Power4.easeInOut }, '+=0.25');
		tl.staggerFrom(p3.words, 1, { opacity: 0 }, 0.05, '-=0.25');

		tl.staggerTo(p0.words, 0.3, { opacity: 0 }, 0.03, '+=1.5');
		tl.staggerTo(p1.words, 0.3, { opacity: 0 }, 0.03, '-=0.27');
		tl.staggerTo(p2.words, 0.3, { opacity: 0 }, 0.03, '-=0.27');
		tl.to(div, 0.3, { x: 50, ease: Power4.easeInOut }, '-=0.27');
		tl.staggerTo(p3.words, 0.3, { opacity: 0 }, 0.03, '-=0');
		tl.call(secondPart);
	}
	...
	function secondPart() {
		...
		document.body.classList.add("show-intro-2");
	...
	function skip() {
		...
		// wait for the outro animation to complete
		setTimeout(function () {
			document.body.classList.remove("show-intro");
			document.body.classList.remove("show-intro-2");
		}, 600);
	...
	function pickPath() {
		...
		var start = document.querySelector('.intro__start');
		start.addEventListener('click', skip);

		if (hasCookie('seenTextIntro')) {
			console.log('2');
			secondPart();
		} else {
			console.log('1');
 ...
 ***************************************************************************/
	...
	function hasCookie(cookie) {
		var hasVisited = getCookie(cookie);
		if (hasVisited === "true") {
			return true;
		} else {
			return false;
...
function LeaderView() {
	...
	function init() {
		node = document.querySelector('.people-view');
		closeBtn = node.querySelector('.close');
	...
	function initSlick() {
		...
		var slickSlider = $(".people-view__list").slick({
			arrows: true,
			centerMode: true,
			slidesToShow: 1,
			infinite: false,
			...
			adaptiveHeight: true,
			accessibility: false,
			prevArrow: '<a class="slick-prev"><svg viewBox="0 0 100 100"><use xlink:href="#arrow-up"></use></svg></a>',
			nextArrow: '<a class="slick-next"><svg viewBox="0 0 100 100"><use xlink:href="#arrow-up"></use></svg></a>'
		});
	...
	function hide(ctx, next) {
		if (slickSlider) {
			$(".people-view__list").slick('unslick');
			slickSlider = null;
		}
	...
	function show() {
		...
		document.body.setAttribute('data-state', 'people');
		resize();
		visible = true;
		...
		if (mode === "desktop") {
			var items = node.querySelectorAll('.people-view__list > li');
			animation = new TimelineLite();
			animation.from(node.querySelector('.close'), 1, { right: -100, opacity: 0, ease: Power4.easeInOut });

			for (var i = 0; i < items.length; i++) {
				var t = 0.5 + 0.2 * i;
				animation.from(items[i], 1, { y: 800, opacity: 0, ease: Power4.easeOut }, t);
			}
		}
	...
	function resize() {
		...
		if (visible) {
			if (prevMode === "mobile" && mode === "desktop") {
				console.log('go desktop');
				$(".people-view__list").slick('unslick');
				slickSlider = null;
			} else if (prevMode === "desktop" && mode === "mobile") {
...
function DetailView(options) {
	...
	// positioning
	var leftMargin = 200; // detail-view has 140px margins on the side
	var startX = 52 + 18; // logo's x + width/2
	var startY = 40 + 18; // logo's y + height/2
	...
	var next; // callback for transition close
	var tranData = {};
	var animation; // page content animation
	var transition; // timeline-detail view tanimation
	var fullShow = false; // big timline-detail view transition
	var ready = false;
	var animReady = false;
	var sfx = { intro: 0, outro: 0 }; // circle tweens
	...
	function init() {
		node = document.querySelector('.detail-view');
	}

	init();

	function update(slug) {
		// clear out previous page's content
		node.innerHTML = '';
		activeSlug = slug;
		...
		if (fullShow) {
			...
			node.innerHTML = '<div class="spinner"><div class="dot1"></div><div class="dot2"></div></div>';
		}
		...
		if (history.hasOwnProperty(activeSlug)) {
			onLoadComplete(history[activeSlug]);
		} else {

			setTimeout(function () {
				//show a spinner if its taking to long
				spinner = document.querySelector('.timeline-animation .spinner');
				if (spinner) {
	...
	function onLoadComplete(raw) {
		console.log("onLoadComplete");
		if (!history.hasOwnProperty(activeSlug)) {
			history[activeSlug] = raw;
		}
		...
		// layout timeline
		node.innerHTML = ich.details(data, true);

		// create html for popup
		popupHTML = [];
		for (var i = 0; i < data.blog.length; i++) {
			var obj = {
				src: ich.popup(data.blog[i], true),
				type: 'inline'
			};
		...
		if (direction == 'horizontal') {
			var delay = fullShow ? 0.3 : 0;

			animation = new TimelineLite({
				paused: true,
				onComplete: deeplinkPopup
			});
			animation.from(node.querySelector('.meta'), 1, { x: 800, opacity: 0, ease: Power4.easeOut }, delay);
			animation.from(node.querySelector('.line'), 1, { x: 800, opacity: 0, ease: Power4.easeOut }, delay + 0.2);
			animation.from(node.querySelector('.close'), 1, { right: -100, opacity: 0, ease: Power4.easeInOut }, delay + 0.2);

			var items = node.querySelectorAll('.blog ul li');
			for (var i = 0; i < items.length; i++) {
				var t = 0.2 + 0.2 * i + delay;
				animation.from(items[i], 1, { x: 800, opacity: 0, ease: Power4.easeOut }, t);
			}
			...
			animation.from(node.querySelector('.meta'), 0.5, { y: 300, opacity: 0, ease: Power4.easeOut }, delay);
			animation.from(node.querySelector('.line'), 0.5, { y: 300, opacity: 0, ease: Power4.easeOut }, delay + 0.1);

			var items = node.querySelectorAll('.blog ul li');

			for (var i = 0; i < items.length; i++) {
				var t = 0.1 + 0.2 * i + delay;
				animation.from(items[i], 0.5, { y: 300, opacity: 0, ease: Power4.easeOut }, t);
			}
		...
		/* adds more height to preview images if it needs it */
		setTimeout(function () {

			var minArtHeight = 230;
			var art = node.querySelectorAll('.blog ul li .preview img');
			var tall = false;

			for (var i = 0; i < art.length; i++) {
				if (art[i].clientHeight > minArtHeight) {
					tall = true;
				}
			}
			...
			if (tall === true) {
				for (var i = 0; i < art.length; i++) {
					art[i].parentNode.parentNode.classList.add("tall");
				}
			}
		}, 100);
	}

	function onWheelUpdate(e) {
		if (mode == 'desktop') {
			e.preventDefault();
			...
			var d = draggable.scrollProxy.left() + delta * 30;
			d = d > 0 ? 0 : d;

			var m = -(draggable.target.scrollWidth - draggable.target.clientWidth);
			d = Math.max(d, m);

			draggable.scrollProxy.left(d);
			draggable.update();
		}
	}

	function createTransition() {
		var anim = document.querySelector('.timeline-animation');
		var meta = anim.querySelector('.meta');
		var canvas = anim.querySelector('.sfx');
		var c = canvas.getContext('2d');

		// set metadata
		meta.innerHTML = '<h1>' + tranData.title + '</h1><h2>' + tranData.year + '</h2><div class="spinner"><div class="dot1"></div><div class="dot2"></div></div>';

		// reset starting positions
		TweenLite.set(meta, { clearProps: "all" });
		TweenLite.set(sfx, { intro: 0, outro: 0 });
		canvas.width = window.innerWidth;
		canvas.height = window.innerHeight;

		// enter
		transition = new TimelineLite({ paused: false });
		transition.set(anim, { display: 'block' });
		transition.to(sfx, 0.7, { intro: 1, ease: Power4.easeInOut, onUpdate: drawSFXCircle });
		transition.call(showText, [], this, 0);
		transition.addPause(null, waitForIt);
		transition.call(hideText);
		transition.to(sfx, 0.7, { outro: 1, ease: Power4.easeInOut, onUpdate: drawSFXCircle });
		transition.set(anim, { display: 'none' });
		transition.addPause();

		// exit
		transition.set(anim, { display: 'block' });
		if (mode === 'mobile') {
			console.log("mobile outro");
			transition.to(sfx, 0.6, { outro: 0, ease: Power4.easeInOut, onUpdate: drawSFXCircle });
			transition.to(sfx, 1.0, { intro: 0, ease: Power4.easeInOut, onStart: nextSection, onUpdate: drawSFXCircle });
		} else {
			transition.to(sfx, 0.6, { outro: 0, ease: Power4.easeInOut, onUpdate: drawSFXCircle });
			transition.call(nextSection, null, null, '-=0.1');
			transition.to(sfx, 0.7, { intro: 0, ease: Power4.easeInOut, onUpdate: drawSFXCircle }, '-=0.1');
		}
		transition.set(anim, { display: 'none' });
	}

	function deeplinkPopup() {
		if (loadPopup && mode == 'desktop' && !fullShow) {
			var id;
			for (var i = 0; i < data.blog.length; i++) {
				if (data.blog[i].id === loadPopup) {
					id = i;
					break;
				}
			}
	...
	function drawSFXCircle() {
		var anim = document.querySelector('.timeline-animation');
		var canvas = anim.querySelector('.sfx');
		var c = canvas.getContext('2d');
		var r1 = Math.round(sfx.intro * Math.max(window.innerWidth, window.innerHeight) * 2);
		var r2 = Math.round(sfx.outro * Math.max(window.innerWidth, window.innerHeight) * 2);

		c.globalCompositeOperation = "source-over";
		c.clearRect(0, 0, canvas.width, canvas.height);
		c.beginPath();
		c.arc(tranData.x, tranData.y, r1, 0, 2 * Math.PI);
		c.fillStyle = '#c52127';
		c.fill();
		...
		if (r2 > 0) {
			...
			c.beginPath();
			c.arc(window.innerWidth / 2, window.innerHeight / 2, r2, 0, 2 * Math.PI);
			c.fill();
			c.closePath();
		}
	}
	...
	function showText() {
		...
		TweenLite.to(meta, 0.3, { autoAlpha: 1, ease: Power2.easeInOut });
		TweenLite.to(meta, 2, { scale: 1.5, delay: 0.2 });
	}
	...
	function hideText() {
		...
		TweenLite.to(meta, 0.5, { autoAlpha: 0 });
	}
	...
	function initEvents() {
		...
		var workNode = node.querySelector('.blog');

		if (draggable) {
			draggable.kill();
		}

		Draggable.create(workNode, { type: "scrollLeft", edgeResistance: 1, throwProps: true });
		draggable = Draggable.get(workNode);

		toggleDraggable();

		// mousewheel
		workNode.addEventListener('mousewheel', onWheelUpdate, false);
		workNode.addEventListener('DOMMouseScroll', onWheelUpdate, false);

		for (var i = 0; i < data.blog.length; i++) {
			var item = workNode.querySelector('li:nth-child(' + (i + 1) + ')');
			item.addEventListener('click', viewLargeContent.bind(this), false);
		}
	...
	function show(slug, ctx) {
		...
		if (ctx.state.id) {
			loadPopup = ctx.state.id;
		} else {
			loadPopup = null;
		}

		if (page.previous === '/' && ctx.state.title !== undefined) {
			fullShow = true;
		} else {
			fullShow = false;
		}
		visible = true;
		ready = animReady = false;
		tranData = ctx.state;

		update(slug);

		document.body.setAttribute('data-state', 'blog');
	}

	function hide(callback) {

		// Remove all events on close
		var items = node.querySelectorAll('.blog ul li');
		for (var i = 0; i < items.length; i++) {
			items[i].removeEventListener('click', viewLargeContent.bind(this), false);
		}

		// reset then remove draggable
		draggable.scrollProxy.left(0);
		draggable.update();
		// Removed for this bug FIFTIETH-212
		// draggable.kill();
	...
	function viewLargeContent(e) {
		if (mode == 'desktop') {
			e.preventDefault();

			showPopup($(e.currentTarget).index());
		}
	}
	...
	function showPopup(index) {
		...
		$.magnificPopup.open({
			...
			gallery: {
				enabled: true,
				arrowMarkup: ich.popupArrow({}, true),
				autoPlay: false
			},
			closeMarkup: ich.popupClose({}, true),
			...
			callbacks: {
				...
				open: function open() {
					...
					$.magnificPopup.instance.next = function () {
						...
						$(".mfp-content").removeClass("outro2").removeClass("intro2").removeClass("intro").addClass("outro");
						var self = this;
						setTimeout(function () {
							$.magnificPopup.proto.next.call(self);
							$(".mfp-content").removeClass("outro").addClass("intro");
						}, 400);
					};

					$.magnificPopup.instance.prev = function () {

						$(".mfp-content").removeClass("outro").removeClass("intro").removeClass("intro2").addClass("outro2");
						var self = this;
						setTimeout(function () {
							$.magnificPopup.proto.prev.call(self);
							$(".mfp-content").removeClass("outro2").addClass("intro2");
						}, 400);
					};

					$.magnificPopup.instance.close = function () {

						$(".mfp-content").removeClass("intro");
	...
	function scrollHandler(e) {
		e.preventDefault();
		window.scrollTo(window.pageXOffset + e.deltaY, window.pageYOffset);
	}
...
function MilestoneView() {
	...
	var node;
	var visible = false;
	var paginationPrevButtons;
	var paginationNextButtons;
	var lists;
	var closeBtn;
	var mode;
	var prevMode;
	...
	var img;
	var animation;
	var assignMilestoneEvents = true;

	function init() {
		node = document.querySelector('.milestone-view');
		paginationNextButtons = node.querySelectorAll('.milestone-view__pagination-btn--next');
		paginationPrevButtons = node.querySelectorAll('.milestone-view__pagination-btn--prev');
		lists = node.querySelectorAll('.milestone-view__sub-list');
		closeBtn = node.querySelector('.milestone-view__close');
		img = node.querySelectorAll('.image-wrapper');
		if (assignMilestoneEvents) {
			initMilestoneEvents();

			assignMilestoneEvents = false;
		}
		initWaypoints();
		if (mode === "mobile") {
			initSlick();
	...
	function initSlick() {
		...
		var slickSlider = $(".milestone-view__list").slick({
			arrows: true,
			centerMode: true,
			slidesToShow: 1,
			infinite: false,
	...
	function initMilestoneEvents() {

		Array.prototype.forEach.call(paginationPrevButtons, function (el, i) {
			el.addEventListener('click', paginationPrevButtonHandler);
		});

		Array.prototype.forEach.call(paginationNextButtons, function (el, i) {
			el.addEventListener('click', paginationNextButtonHandler);
		});
	...
	function updateScroll() {
		Array.prototype.forEach.call(lists, function (el, i) {
			...
			if (el.offsetHeight < ul.offsetHeight) {
				el.parentNode.querySelector('.milestone-view__pagination-btn--next').classList.add('show');
			} else {
				el.parentNode.querySelector('.milestone-view__pagination-btn--next').classList.remove('show');
				el.parentNode.querySelector('.milestone-view__pagination-btn--prev').classList.remove('show');
			}
		});
	}

	function paginationPrevButtonHandler(el) {
		el.preventDefault();

		var scrollHeight = el.target.parentNode.querySelector('.milestone-view__sub-list').offsetHeight;
		var currentMarginTop = window.getComputedStyle(el.target.parentNode.querySelector('.milestone-view__sub-list-ul')).marginTop;

		var newMarginTop = Math.max(0, Math.abs(currentMarginTop.replace("px", "")) - scrollHeight);
		el.target.parentNode.querySelector('.milestone-view__sub-list-ul').style.marginTop = -newMarginTop + "px";

		if (newMarginTop === 0) {
			el.target.classList.remove('show');
		}

		el.target.parentNode.querySelector('.milestone-view__pagination-btn--next').classList.add('show');
	...
	function paginationNextButtonHandler(el) {
		el.preventDefault();

		var scrollHeight = el.target.parentNode.querySelector('.milestone-view__sub-list').offsetHeight;
		var totalHeight = el.target.parentNode.querySelector('.milestone-view__sub-list-ul').offsetHeight;
		var currentMarginTop = window.getComputedStyle(el.target.parentNode.querySelector('.milestone-view__sub-list-ul')).marginTop;

		var newMarginTop = Math.abs(currentMarginTop.replace("px", "")) + scrollHeight;
		el.target.parentNode.querySelector('.milestone-view__sub-list-ul').style.marginTop = -newMarginTop + "px";

		el.target.parentNode.querySelector('.milestone-view__pagination-btn--prev').classList.add('show');

		if (totalHeight - newMarginTop < scrollHeight) {
			el.target.classList.remove('show');
		}
	...
	function hide(ctx, next) {
		...
		if (slickSlider) {
			$(".milestone-view__list").slick('unslick');
			slickSlider = null;
		}
	...
	function show() {
		...
		document.body.setAttribute('data-state', 'milestone');
		resize();
		visible = true;
		init();
		node.classList.add('show');
		...
		if (mode === "desktop") {
			...
			var items = node.querySelectorAll('.milestone-view__list > li');
			animation = new TimelineLite();
			animation.from(node.querySelector('.milestone-view__close'), 1, { right: -100, opacity: 0, ease: Power4.easeInOut });
...
function SearchView() {
	...
	function init() {
		node = document.querySelector('.search');

		// listen for user input
		query = node.querySelector('.search__input input');
		query.addEventListener('input', onChange, false);

		// fancy line
		line = node.querySelector('.search__line');
		spinner = node.querySelector('.spinner');

		// empty search results
		results = node.querySelector('.search__results');

		// search results
		$('.search__results').on('click', '.search-results > li', function (e) {
			var data = {};
			data.id = e.currentTarget.getAttribute('data-id') || undefined;
			page('/blog/' + e.currentTarget.getAttribute('data-url'), data);
		});

		// dont want the page to change
		$('.search__input form').submit(function (e) {
			e.preventDefault();
		});

		// close
		$('.close', node).on('click', exit);
	}
	...
	function createAnimation() {
		if (mode === "desktop") {
			...
			animation = new TimelineLite();
			animation.from(query, 1, { x: 800, opacity: 0, ease: Power4.easeOut });
			animation.from(line, 1, { x: 800, opacity: 0, ease: Power4.easeOut }, 0.2);
			animation.from(node.querySelector('.close'), 1, { right: -100, opacity: 0, ease: Power4.easeInOut }, 0.2);

			var items = results.querySelectorAll('ul.search__categories > li');
			for (var i = 0; i < items.length; i++) {
				var t = 0.2 + 0.2 * i;
				animation.from(items[i], 1, { x: 800, opacity: 0, ease: Power4.easeOut }, t);
			}
 ...
 ***************************************************************************/
	function onChange(e) {
		if (e.currentTarget.value !== '') {
			...
			// prevent the server from being crushed
	...
	function clearSearchResults() {
		...
		spinner.classList.remove("show");
		results.innerHTML = ich.search(fake, true);
	}

	function onLoadComplete(response) {
		spinner.classList.remove("show");

		var noResults = node.querySelector('.search__results-none');
		...
		results.innerHTML = ich.search(response, true);
	}
	...
	function show() {
		...
		visible = true;
		node.classList.add('show');
		document.body.setAttribute('data-state', 'search');
		resize();
		query.focus();
		createAnimation();
	}
...
function Entry(x, y, data, el) {

	var entryWidth = 200;
	var entryLabelTopMargin = 0;
	var url;
	var node;
	...
	function init(x, y, data, el) {
		url = data.url;
		var tmp = ich.blog(data, true);
		...
		node.style.top = y + 'px';
		node.addEventListener('click', view);
	...
	init(x - entryWidth / 2, y + entryLabelTopMargin, data, el);
	...
	function hide() {
		...
		node.className = node.className.replace(/\sshow\s/g, '');
	}
...
function Era(options) {
	var data;
	var firstIsMilestone = false;

	var orbRadius = 60;
	var orbMargin = 20;
	var orbShift = orbRadius + orbMargin;
	...
	var strandWidth;
	var strandHeight = 400;

	var orgX;
	var orgY;
	var left; // left most point on strand
	var right; // right most point on strand
	var offset = 0;
	var offset2 = 0;

	var state = 'collapsed'; // expanded, collapsed, expanding, collapsing
	var showingEntries = false;
	...
	var to;
	var animationExpand;
	var animationCollapse;

	var animation;
	var midLine = { progress: 0, start: 0, end: 0 };
	var midPointLocked = false;
	var pauseTween = { x: 0 };

	function init() {
		data = options.data;
		strandWidth = options.width;

		// using entryBuffer on front and back plus normal width
		expandWidth = entryWidth * options.count + entryBuffer * 2;

		speed = Math.max(options.count / 13, speed);

		options.expandWidth = expandWidth;
		...
		// setup strand physics
		options.x += orbShift;
		options.width -= 2 * orbShift;
		strand = new Strand(options);

		// create elements
		createOrb(options.canvas);
		createWrappers();
		createAnimation();
		createEntries();
	...
	/***************************************************************************
 *                                                                          *
 *                        Setup Elements within Era                         *
 *                                                                          *
 ***************************************************************************/
	function createWrappers() {
		milestoneContainer = document.createElement('div');
		milestoneContainer.className = 'milestones';

		workContainer = document.createElement('div');
		workContainer.className = 'blog';

		entriesContainer = document.createElement('div');
		entriesContainer.className = 'entries';
		entriesContainer.style.display = 'none';
		entriesContainer.appendChild(milestoneContainer);
		entriesContainer.appendChild(workContainer);

		var wrapper = document.querySelector('.timeline .timeline-wrapper');
		wrapper.appendChild(orbContainer);
		wrapper.appendChild(entriesContainer);
	}

	function createOrb(c) {
		hitArea = document.createElement('div');
		hitArea.className = 'orb__hit-area';
		hitArea.addEventListener('click', orbClick);
		...
		var center = orb.getCenterOffset();

		// attach orbContainer
		orbContainer = orb.getElement();
		orbContainer.style.left = orgX - center.x + 'px';
		orbContainer.appendChild(hitArea);
	}

	function createAnimation() {
		animation = new TimelineLite({
			paused: true,
			onComplete: expandComplete,
			onReverseComplete: collapseComplete
		});
		animation.to(midLine, 1.3, { progress: 1, onUpdate: updateMidPoint, onComplete: lockMidPoint, onReverseComplete: unlockMidPoint });
		animation.add(orb.getAnimation().play());
		animation.addPause(null, isCollapsed);
		animation.to(pauseTween, 0.1, { x: 1 });
	}

	function createEntries() {
		var center = orb.getCenterOffset();
		var p = strand.bodies[0].getPosition();

		var w = 0; // blog within timeline
		var lookAhead = true; // should we count the # of milestones
		var milestones = 1; // milestones in between 2 blog items
		var m = 1; // current milestone b/w blog items
		var offset = entryBuffer;

		var animDelay = animation.duration() - 0.6;
		var entryDelay = 0.3;
		var dur = data.count < 11 ? (data.count - 1) * entryDelay : 10 * entryDelay;

		for (var i = 0; i < data.entries.length; i++) {
			var x, y, entry, delay, a;

			if (data.entries[i].type == 'blog') {
				// blog
				x = entryWidth + entryWidth * w + offset;
				y = 0;
				delay = w < 10 ? w * entryDelay : 10 * entryDelay;
				a = dur * tlEase(delay, 0, 1, dur);

				entry = new Entry(x, y, data.entries[i], workContainer);

				animation.set(entry.getNode(), { css: { className: '+=show' } }, animDelay + a);
				...
				if (lookAhead) {
					for (var j = i; j < data.entries.length; j++) {
						if (data.entries[j].type == 'blog') {
							break;
						} else {
							milestones++;
						}
				...
				// milestones
				md = entryDelay / (milestones + 1) * 5;
				x = entryWidth + entryWidth * (w - 1) + offset + m * (entryWidth / milestones);
				y = center.y - 50 - Math.round(Math.sin((m - 1) * 1.4) * 20);
				delay = w < 10 ? w * entryDelay + m * md : 10 * entryDelay + m * md;
				a = dur * tlEase(delay, 0, 1, dur);

				entry = new MilestoneEntry(x, y, data.entries[i], milestoneContainer);

				animation.set(entry.getNode(), { css: { className: '+=show' } }, animDelay + a);
				animation.set(entry.getNode(), { css: { className: '+=bubble' } }, animDelay + a + 1.5);
 ...
 ***************************************************************************/
	function orbClick() {
		if (state == 'collapsed' || state == 'collapsing') {
			options.fn(options.id, 'expand');
		} else if (state == 'expanded' || state == 'expanding') {
			options.fn(options.id, 'collapse');
		}
	}
 ...
 ***************************************************************************/
	function render() {
		// animation orb position
		var t = orb.getCenterOffset();
		ctx.drawImage(orb.render(), orgX + offset - t.y, 0); // orb
		orbContainer.style.left = orgX + offset - t.y + 'px'; // orb's hit area
		entriesContainer.style.left = orgX + 'px';

		// update thread rendering
		renderThread();
	}

	function renderThread() {
		// reset ends to be "fixed"
		strand.bodies[0].setPosition(strand.left);

		if (midPointLocked) {
			var t = strand.bodies[1].getPosition();
			var middle = {
				x: t.x,
				y: strand.left.y,
				z: t.z
			};
			strand.bodies[1].setPosition(middle);
		}

		strand.bodies[2].setPosition(strand.right);
		strand.render();
	}
 ...
 ***************************************************************************/
	// Animation methods
	function move(left, right, expand) {
		offset = left;
		offset2 = right > left ? right : 0;
 ...
 */
	function open() {
		...
		midLine.end = strand.bodies[0].getPosition().y;
		midLine.start = strand.bodies[1].getPosition().y;

		TweenLite.set(entriesContainer, { display: 'block' });

		animation.timeScale(speed / 2);
		animation.play();
	}
 ...
 */
	...
	function expandComplete() {
		state = 'expanded';
		options.fnExpandedCallback(options.id);
	}

	function collapseComplete() {
		state = 'collapsed';
		TweenLite.set(entriesContainer, { display: 'none' });
		options.fnCollapsedCallback();
	}
	...
	function updateMidPoint() {
		if (animation.reversed()) {
			...
			var diff = midLine.start - midLine.end;
			var temp = strand.bodies[1].getPosition();
			var first = strand.bodies[0].getPosition();
			temp.y = diff * (1 - midLine.progress) + midLine.end;

			if (temp.x < first.x) {
				temp.x += 18;
			}

			strand.bodies[1].setPosition(temp);
		}
	}
	...
	function unlockMidPoint() {
		midPointLocked = false;
	}
 ...
 */
	...
	function createLegendTimeline() {
		...
		divContainer.id = "current-era-text";

		//creates and li for each era
		$.each(data, function (index, value) {
			data[index].count = index;

			data[index].startshort = String(data[index].start).slice(-2);
			data[index].stopshort = String(data[index].stop).slice(-2);

			var tmpText = ich.footerTextTemplate(data[index], true);
			var div = document.createElement('div');
			div.innerHTML = tmpText;
			divContainer.appendChild(div);

			var tmp = ich.footerLinksTemplate(data[index], true);
			var li = document.createElement('li');
			li.innerHTML = tmp;

			//loop through all the entries within an era
			var eraTimelineUl = document.createElement('ul');

			var years = [];

			$.each(data[index].entries, function (index, value) {

				var year = String(value.year).slice(-2);

				if ($.inArray(year, years) < 0) {

					//create an li and add a class depending on the type
					//it will either be blog or milestone
					var li2 = document.createElement('li');
					li2.setAttribute('data-year', value.year);
					...
					eraTimelineUl.appendChild(li2);
				}
			});

			li.insertBefore(eraTimelineUl, li.firstChild);
	...
	function initEvents() {
		...
		var eraButtons = el.querySelectorAll('.era');
		var yearButtons = el.querySelectorAll('ul > li > ul >li');

		Array.prototype.forEach.call(eraButtons, function (el, i) {
			el.addEventListener('click', eraButtonHandler);
		});

		Array.prototype.forEach.call(yearButtons, function (el, i) {
			el.addEventListener('click', eraButtonYearHandler);
		});
	}

	function eraButtonHandler(el) {

		console.log(el.target);
		console.log("new era " + Number(el.target.getAttribute('data-era')));
		inactive();

		if (el.target.parentNode.classList.contains("active-era")) {
			options.callbackClose(Number(el.target.getAttribute('data-era')));
		} else {
			options.callbackOpen(Number(el.target.getAttribute('data-era')));
		}
	}

	function eraButtonYearHandler(el) {

		options.callbackYear(el.target.getAttribute('data-year'));
		updateYear(el.target.getAttribute('data-year'));
	}
	...
	function resize() {
		...
		//set the padding of the era timeline so its starts where the a tag ends
		var activeEraA = document.querySelector(".legend > ul > li.active-era > a");
		var activeEraUL = document.querySelector(".legend > ul > li.active-era > ul");
		var activeEraLI = document.querySelector(".legend > ul > li.active-era");
		var activeEraP = document.querySelector(".legend > ul > li.active-era p");
		var eraLIs = document.querySelectorAll(".legend > ul > li");

		if (activeEraUL && activeEraP) {
			activeEraUL.style.paddingLeft = activeEraA.offsetWidth + "px";

			if (mode === "mobile") {
				activeEraLI.classList.remove("active-era-desktop");
			} else {
				activeEraLI.classList.add("active-era-desktop");
			}
		}
	}
	...
	function setActiveEra(era) {
		...
		console.log("LEGEND setActiveEra = " + era);

		activeEra = era;

		if (document.querySelector('.legend > ul li.active-year')) {
			document.querySelector('.legend > ul li.active-year').classList.remove("active-year");
			var processBar = document.querySelector(".legend > ul > li.active-era .progress-bar");
			processBar.style.width = "";
		}

		var eraText = document.querySelectorAll(".legend > div > div");
		var eraLIs = document.querySelectorAll(".legend > ul > li");

		Array.prototype.forEach.call(eraLIs, function (el, i) {
			el.classList.remove("active-era");
			el.classList.remove("active-era-desktop");

			if (i === activeEra) {
				el.classList.add("active-era");
		...
		Array.prototype.forEach.call(eraText, function (el, i) {
			el.classList.remove("active-era");
			if (i === activeEra) {
				el.classList.add("active-era");
			}
	...
	function closeActiveEra() {

		console.log("LEGEND closeActiveEra");
	...
	function updateYear(year) {
		...
		var processBar = document.querySelector(".legend > ul > li.active-era .progress-bar");
		var activeA = document.querySelector('.legend > ul > li.active-era > a');
		var yearBar = document.querySelector('.legend > ul > li.active-era li[data-year="' + year + '"]');
		if (yearBar) {
			yearBar.classList.add("active-year");
			processBar.style.width = yearBar.offsetLeft - activeA.offsetWidth / 2 + 20 + "px";
		}
	}
...
function MilestoneEntry(x, y, data, el) {
	...
	function init(x, y, data, el) {
		...
		var tmp = ich.milestone(data, true);
		var div = document.createElement('div');
		div.innerHTML = tmp;
	...
	function show(delay) {
		to1 = setTimeout(function () {
			...
			to2 = setTimeout(function () {
				node.className = node.className.replace(/\sshow\s/g, ' bubble ');
			}, 1550);
		}, delay * 1000);
	}
	...
	function hide() {
		clearTimeout(to1);
		clearTimeout(to2);
		node.className = node.className.replace(/\s(show|bubble)\s/g, '');
	}
...
*/
function MobileTimeline(options) {
 ...
 ***************************************************************************/
	/**
 * Setup the main timeline interface
 */
	function onLoadComplete(data) {
		node = document.querySelector('.mobile-timeline');
	...
	/**
 * If we can't load the initial json, then we're in trouble
 */
 ...
 ***************************************************************************/
	function layoutTimeline(data) {
		...
		for (var i = 0; i < data.length; i++) {
			data[i].index = i;
			html += ich.era(data[i], true);
		}
		html += ich.eraClose({}, true);

		node.innerHTML = html;

		// Creat SVG for strand animation
		svgContainer = document.createElement('div');
		svgContainer.id = 'mobile-timeline__strand';
		node.appendChild(svgContainer);

		var svg = SVG('mobile-timeline__strand');
		strand = svg.path('M0 0 L-1 -1');
		strand.fill('none').style({
			stroke: '#c52127',
			'stroke-width': 1
		});
	}
	...
	function drawStrand() {
		var abort = false;
		var path = '';
		var w = Math.min(document.body.clientWidth, 768);
		...
		var eras = node.querySelectorAll('.mobile-timeline__era');
		var side = Math.round((w - 200) / 4);
		side = side > 80 ? side + 60 : side;
		var coords = [];

		$('.mobile-timeline__era').each(function (index, el) {
			if ($(el).offset().top == 0) {
				abort = true;
			}
			var y = $(el).offset().top + 200;
			var x = index % 2 === 1 ? side : w - side;
			var e = 27;

			if (index === 0) {
				coords.push({ x: start.x, y: y - 200 });
			}

			coords.push({ x: x, y: y, e: e });
		});
		...
		for (var o = 0; o < offset.length; o++) {
			coords[o + 1].x = coords[o + 1].x + (coords[o + 1].e - coords[o + 1].x) * offset[o].x;
		}
		...
		for (var c = 1; c < coords.length; c++) {
			var y = coords[c].y;
			var x = coords[c].x;
			var y2 = coords[c].y - 200;
			m = $('.mobile-timeline__era:nth-child(' + c + ') .content').height();

			path += bezierCurveTo(x2, y2, x, y2, x, y);
			path += lineTo(x, y + m);

			x2 = x;
		}
		x = w - side;
		y = coords[coords.length - 1].y + 400 + m;
		y2 = y - 200;
	...
	function setupOffsets() {
		offset = [{ x: 0 }, { x: 0 }, { x: 0 }, { x: 0 }];
	}

	function setupEvents() {
		var orbs = node.querySelectorAll('.mobile-timeline__era .orb');
		for (var i = 0; i < orbs.length; i++) {
			orbs[i].addEventListener('click', orbClick);
		}

		var exit = node.querySelector('.mobile-timeline .close');
		exit.addEventListener('click', closeClick);

		$('.mobile-timeline .entry--blog').on('click', showWork);
	}

	function setupWaypoints() {

		var eras = node.querySelectorAll('.mobile-timeline__era .label');

		Array.prototype.forEach.call(eras, function (el, i) {
			...
			var waypoint = new Waypoint({
				...
				handler: function handler(direction) {
					...
					if (direction === "down") {
						this.element.classList.add('in-frame');
					}
				},
				offset: '90%'
			});
			...
			var waypoint2 = new Waypoint({
				...
				handler: function handler(direction) {
					...
					this.element.classList.remove('in-frame');
				},
				offset: '100%'
			});
		});
	...
	function showWork(e) {
		...
		page.show(url, coords);
	}

	function orbClick(e) {
		var era = Number(e.currentTarget.getAttribute('data-era'));
		openEra(era);
	}
	...
	function openEra(era, forceOpen) {

		if (!animating) {

			console.log("openEra");

			if (activeEra == null || forceOpen) {
				animating = true;
				nextEra = null;

				var $era = $('.mobile-timeline__era:nth-child(' + (era + 1) + ')');

				TweenLite.to(window, 1, { scrollTo: { y: $era.offset().top + 100 } });

				TweenLite.set($era.find('.content'), { height: 'auto' });
				var h = $era.find('.content').height();
				TweenLite.set($era.find('.content'), { height: 0 });
				...
				animation.to(offset[era], 0.6, { x: 1 });

				if (era % 2 == 1) {
					animation.to($era.find('.meta'), 0.6, { x: '100%' }, 0);
				} else {
					animation.to($era.find('.meta'), 0.6, { x: '-100%' }, 0);
				}

				animation.to($era.find('.content'), 1.2, { height: h });

				activeEra = era;

				node.querySelector('.mobile-timeline .actions').classList.add('show');
			} else {
				nextEra = era;
				closeEra(activeEra, true);
			}
		}
	}

	function closeEra(era, skip) {

		console.log("closeEra");

		if (!animating) {

			console.log("closeEra not animating");
			...
			if (!skip) {
				node.querySelector('.mobile-timeline .actions').classList.remove('show');
			}
		}
	}
 ...
 ***************************************************************************/
	...
	function show(ctx) {
		...
		if (page.previous.indexOf('/s') >= 0 && lastScroll !== null) {
			TweenLite.to(window, 0, { scrollTo: { y: lastScroll } }, 0.3);
			lastScroll = null;
		}
		...
		try {
			...
			if (ctx.state.era !== undefined) {
				if (Number(ctx.state.era) != activeEra) {
					console.log(ctx.state.era);
					openEra(Number(ctx.state.era));
				}
			}
		} catch (err) {}
	}
...
function Orb(options) {
	...
	var red = '#c52127'; // standard red
	var stroke = '#e1e1e1'; // approximate background
	...
	function init() {
		...
		// find size of canvas
		var size = 2 * (options.radius + options.margin + padding);
		...
		// set center point
		center.x = options.radius + options.margin + padding;
		center.y = options.radius + options.margin + padding;

		// store radius info
		radius = options.radius;
		outerRadius = options.radius + options.margin;
	...
	function layoutCategoryOrbs() {
		...
		for (var val in categories) {
			...
			// combine print and outdoor
			if (val == 'print' && categories['outdoor']) {
				mod = categories['outdoor'];
			}

			var size = categories[val];
			maxSize = Math.max(maxSize, size);
		}

		var order = ['print', 'tv', 'digital'];
		for (var i = 0; i < order.length; i++) {
			var prop = order[i];
			var mod = 0;

			// combine print and outdoor
			if (prop == 'print' && categories['outdoor']) {
				mod = categories['outdoor'];
			}
			if (categories[prop]) {
				var cat = categories[prop] + mod;
				if (cat > 0) {
					var s = Math.round(10 * cat / maxSize + 10) / 20;
					div++;

					orbs.push(createCategoryOrb(prop, s));
				}
			}
		}

		chaser = createCategoryOrb('chaser', 0.5);

		var speed = 1.0;
		var spacing = 180 / (orbs.length + 1);
		var timeSlice = speed / orbs.length;
		for (var i = 0; i < orbs.length; i++) {
			var deg = (i + 1) * spacing;
			var delay = timeSlice * i;
			var duration = speed - delay;

			TweenLite.set(orbs[i].category, { rotation: deg });
			TweenLite.set(orbs[i].dot, { rotation: -deg });

			animation.to(orbs[i].category, speed, { rotation: 180, ease: Power4.easeInOut }, 0);
			animation.to(orbs[i].art, duration, { scale: 0.2 }, delay);
			animation.set(orbs[i].category, { opacity: 0 }, duration + delay);
		}

		// Merged blob
		TweenLite.set(chaser.category, { rotation: spacing });
		animation.to(chaser.category, speed, { rotation: 180, ease: Power4.easeInOut }, 0);
		animation.to(chaser.art, speed, { scale: 1, opacity: 1 }, 0);
	}

	function createCategoryOrb(prop, s) {
		var category = document.createElement('div');
		category.className = 'category category--' + prop;
		category.setAttribute('data-size', s);

		var dot = document.createElement('div');
		dot.className = 'dot';

		var art = document.createElement('div');
		art.className = 'dot__art';
		//art.style[transform] = 'translate(-50%,-50%) scale('+s+')';
		TweenLite.set(art, { x: '-50%', y: '-50%', scale: s });
	...
	function render() {
		...
		// draw middle orb
		drawCircle(ctx, center.x, center.y, radius, red);

		// draw outer arc
		ctx.beginPath();
		ctx.arc(center.x, center.y, outerRadius, Math.PI, 2 * Math.PI);
		ctx.lineWidth = 1;
		ctx.strokeStyle = red;
		ctx.stroke();

		// draw left nub
		drawCircle(ctx, padding, center.y, nubRadius, red);

		// draw right nub
		drawCircle(ctx, center.x + outerRadius, center.y, nubRadius, red);
...
function Strand(options) {
	...
	function init(world) {
		...
		point1x = options.x;
		offsetY = options.y - 90;

		point2 = {
			x: options.width / 2 + options.x,
			y: options.y,
			z: 100 * id
		};

		point3 = {
			x: options.width + options.x,
			y: options.y,
			z: 100 * id
		};
		point3x = options.width + options.x;

		weightMiddle = (32 + 10 * options.weight) * .90;

		// create objects
		body.push(createBody(world, 'start' + id, point1, weightEnds));
		body.push(createBody(world, 'middle' + id, point2, weightMiddle));
		body.push(createBody(world, 'finish' + id, point3, weightEnds));

		// create links
		link.push(createLink(world, 'start' + id, 'middle' + id, weightEnds, weightMiddle));
		link.push(createLink(world, 'finish' + id, 'middle' + id, weightEnds, weightMiddle));
	...
	function render() {
		// weighted thread
		var p1 = body[0].getPosition();
		var p2 = body[1].getPosition();
		...
		ctx.bezierCurveTo(a.x, a.y, n2.x + p, p2.y - offsetY, n3.x, n3.y - offsetY);
		ctx.lineWidth = 1;
		ctx.strokeStyle = '#c52127';
		ctx.stroke();
	}
	...
	function createBody(world, name, point, size) {
		return new OIMO.Body({
			world: world,
			size: [size, size, size],
	...
	function createWall(world, name, point, size) {
		return new OIMO.Body({
			world: world,
			size: [1, 10000, 10000],
			pos: [point.x, point.y, point.z],
	...
	function createLink(world, body1, body2, mass1, mass2) {
		return new OIMO.Link({
			world: world,
			type: 'jointHinge',
			body1: body1,
			body2: body2,
	...
	function move(left, right) {
		point1.x = point1x + left;
		point3.x = point3x + right;
	}

	function nudge(direction) {
		var p = body[1].getPosition();
		p.x += direction * -3;

		body[1].setPosition(p);
	}
...
function Timeline(options) {
	...
	var node; // timeline DOM node

	// main timeline strand
	var canvas;
	var ctx;

	// starting strand for the timeline
	var beginningCanvas;
	var beginCtx;
 ...
 */
	...
	/***************************************************************************
 *                                                                          *
 *                          Expand & Collapse Eras                          *
 *                                                                          *
 ***************************************************************************/
	/**
 * When an Era Orb is clicked on the timeline
 */
	function eraClick(id, action) {
		if (action === 'expand') {
			openEra(id);
		} else {
			closeEra(id);
	...
	function openEra(id, fromLegend) {
		if (state === 'collapse') {
			console.log('open from collapse', id);
			activeWidth = era[id].width('expand');
			activeSpeed = era[id].getSpeed();
			activeEra = id;
			waitingToExpand = false;
			...
			animation = new TimelineLite({
				onReverseComplete: continueAnimation,
				onReverseCompleteScope: this,
				onComplete: timelineExpandComplete,
				onCompleteScope: this
			});
			animation.to(expand, activeSpeed, { bezier: [{ x: -30 }, { x: activeWidth }], ease: Power4.easeInOut });

			state = 'expanding';

			era[activeEra].open();
			legend.setActiveEra(activeEra);
			legend.inactive();
		} else if (state == 'expand' && !waitingToExpand) {
			console.log('open from expand', id);
	...
	function timelineExpandComplete() {
		state = 'expand';
		legend.setActiveEra(activeEra);
		legend.active();
	}

	function closeEra(id, fromLegend) {
		console.log('closeEra', id);
		if (state === 'expand') {
			console.log('close era', id);
			if (activeEra !== id) {
				waitingToExpand = true;
			}

			era[activeEra].close();
			legend.closeActiveEra();
		}
	}

	function startEraCollapse() {
		state = 'collapsing';
		...
		var ghostScroll = {
			scrollLeft: draggable.scrollProxy.scrollLeft()
		};

		// animate scroll so we dont get a snap back
		TweenLite.to(ghostScroll, 0.7, {
			scrollLeft: 0,
			onUpdate: function onUpdate() {
				draggable.scrollProxy.scrollLeft(ghostScroll.scrollLeft);
			},
	...
	function onEraExpanded(id) {
		onTimelineMove();
	}

	function onEraCollapsed() {
		console.log('onEraCollapsed');
	...
	function continueAnimation() {
		console.log('continueAnimation - collapsed', activeEra);
		state = 'collapse';
		era[activeEra].close();
	}
 ...
 ***************************************************************************/
	function render() {
		if (renderable) {
			...
			// Recenter
			var eraOffset = Math.max(100, (window.innerWidth - era.length * 200) / 2 + 80);

			for (var i = 0; i < era.length; i++) {
				era[i].shift(i * 200 + eraOffset);
			}
			...
			for (var i = 0; i < era.length; i++) {
				...
				if (i === activeEra) {
					segment.move(0, expand.x);
				} else if (i > activeEra) {
					segment.move(expand.x, expand.x);
				}
			...
			var w = Math.max(window.innerWidth, width + eraOffset + 200);
			if (w != canvas.width) {
				canvas.width = w;
			}

			// Canvas clears content during width change, so render afterwards
			for (var i = 0; i < era.length; i++) {
				var segment = era[i];
				segment.render();
			}
	...
	function drawStartingStrand() {
		var orb = document.querySelector('.timeline .orb');
		...
		var t = orb.getBoundingClientRect();
		var p4 = {
			x: t.left + 10,
			y: t.top + 90
		};

		var diff = Math.max((p4.x - p1.x) / 6, 20);
		...
		beginCtx.clearRect(0, 0, 1200, 2000);

		beginCtx.beginPath();
		beginCtx.moveTo(p1.x, p1.y);
		beginCtx.bezierCurveTo(p2.x, p2.y, p3.x, p3.y, p4.x, p4.y);
		beginCtx.lineWidth = 1;
		beginCtx.strokeStyle = '#c52127';
		beginCtx.stroke();
	}
 ...
 * Show the timeline
 */
	function show(forceClose) {
		visible = true;
		document.body.setAttribute('data-state', 'timeline');

		if (forceClose && activeEra !== null) {
			closeEra(activeEra);
		}
 ...
 */
	...
	function resize() {
		...
		if (visible && mode == 'desktop') {
			cancelAnimationFrame(renderRequest);

			try {
				canvas.width = window.innerWidth;
			} catch (err) {
				console.log('error', canvas.width, window.innerWidth);
			}
			render();
	...
	/***************************************************************************
 *                                                                          *
 *                      Setup Large Screen Environment                      *
 *                                                                          *
 ***************************************************************************/
	function setupEnvironment() {
		// Setup timeline
		var realNode = document.querySelector('.timeline');
		node = document.createElement('div');
		node.className = 'timeline-wrapper';
		node.style.position = 'absolute';
		realNode.appendChild(node);

		// Setup 3d world (oimo.js)
		...
		canvas.width = window.innerWidth;
		canvas.height = 330;
		canvas.className = 'timeline__art';
		node.appendChild(canvas);
		ctx = canvas.getContext('2d');

		// beginning strand
		beginningCanvas = document.createElement('canvas');
		beginningCanvas.className = 'timeline-start';
		beginningCanvas.width = 1200;
		beginningCanvas.height = 2000;
		document.body.appendChild(beginningCanvas);
		beginCtx = beginningCanvas.getContext('2d');

		// make timeline draggable
		Draggable.create(realNode, { type: "scrollLeft", edgeResistance: 1, overshootTolerance: 50, throwProps: true });
		draggable = Draggable.get(realNode);
		draggable.addEventListener('drag', onTimelineMove);

		// mousewheel
		node.addEventListener('mousewheel', onWheelUpdate, false);
		node.addEventListener('DOMMouseScroll', onWheelUpdate, false);
	}

	function setupEras(grouping) {
		var max = 0;
		var cat = 0;

		// find the max # of items in an era
		for (var i = 0; i < grouping.length; i++) {
			max = Math.max(max, grouping[i].count);
			cat = Math.max(cat, grouping[i].categories.outdoor, grouping[i].categories.digital, grouping[i].categories.tv, grouping[i].categories.print);
		}

		// 80 is the orbShift from the era object
		initEraOffset = Math.max(100, (window.innerWidth - grouping.length * 200) / 2 + 80);

		// create eras
		for (i = 0; i < grouping.length; i++) {
			isFirst = i == 0;
			isLast = i == grouping.length - 1;

			era.push(new Era({
				weight: grouping[i].count / max,
				count: grouping[i].count,
				largestCategorySize: cat,
				world: world,
				data: grouping[i],
				id: i,
				x: i * 200 + initEraOffset,
				y: 300,
				width: 200,
				fn: eraClick,
				fnCollapse: startEraCollapse,
				fnExpandedCallback: onEraExpanded,
				fnCollapsedCallback: onEraCollapsed,
				canvas: canvas,
				first: isFirst,
				last: isLast
			}));
	...
	function setupLegend(data) {
		...
		legend = new Legend({
			data: data,
			container: container,
			callbackClose: closeEra,
			callbackOpen: openEra,
			callbackYear: gotoYear
		});
	}
 ...
 ***************************************************************************/
	/**
 * Allow the scroll wheel to move the timeline
 */
	function onWheelUpdate(e) {
		e.preventDefault();
	...
	/**
 * Method that sends the largest viewable year to the legend component
 */
	function onTimelineMove(e) {
		var d = draggable.scrollProxy.scrollLeft() - lastPosition;
		var delta = 0;
		...
		lastPosition = draggable.scrollProxy.scrollLeft();
		if (delta !== 0) {
			for (var i = 0; i < era.length; i++) {
				era[i].nudge(delta);
			}
		...
		if (state == 'expand') {
			var entries = era[activeEra].getNode().querySelectorAll('.entry');
			var viewableArea = window.innerWidth;
			var lastVisibleYear = null;
			var lastVisibleX = 0;

			for (var i = 0; i < entries.length; i++) {
				var left = entries[i].getBoundingClientRect().left;
				var year = entries[i].getAttribute('data-year');
				if (left < viewableArea - 49 && left > lastVisibleX && year > lastVisibleYear) {
					lastVisibleYear = year;
					lastVisibleX = left;
				}
			}

			legend.updateYear(lastVisibleYear);
		}
	}
	...
	function gotoYear(year) {
		...
		for (var i = 0; i < entries.length; i++) {
			var y = entries[i].getAttribute('data-year');
			if (y == year) {
				// 100 is half the entry width
				d = draggable.target.clientWidth - parseInt(era[activeEra].getNode().style.left, 10) - $(entries[i]).position().left - 50;
				d = d > 0 ? 0 : d;
				break;
			}
		}
